...
export class SnookerComponent {
  ...
  objektumFeltolto(feltoltendoElem: string[]): snookerElem[] {
    let beolvasottAdatok: snookerElem[] = [];
  ...
  megjelenitendoAdatok = this.objektumFeltolto(this.snookerInfo);

}

export interface snookerElem {
  helyezes: number;
  nev: string;
  orszag: string;
  nyeremeny: number
